...
function displayBooks() {
    library.forEach((book, index) => {
        ...
        const delBookBtnHTML = `<button data-index="${index}" class="">Delete</button>`;
        ...
        `
        <div class="card" data-indexx="${index}">
        ...
        const delBookBtn = document.querySelectorAll(`[data-index="${index}"]`);
        delBookBtn[0].addEventListener('click', deleteBook);
...
function toggleReadFn(e) {
    console.log(e.target.parentNode.dataset.indexx);
}

// Deletes book from library
function deleteBook(e) {
    bookIndex = e.target.attributes['data-index'].value;
